...
def write_results(prediction, confidence, num_classes, nms_conf = 0.4):
  """
  Outputs a tensor of shape D x 8 where D is the true detections
  in all of images, each represented by a row.

  Each detection has 8 attributes: index if the image in the batch
  to which the detection belongs to, 4 corners coordinates, 
  objectness score, the score of class with max confidence, 
  index of that class. 
  ...
  for ind in range(batch_size):
    ...
    # perform NMS
      ...
      # in the loop
        try:
          # first input is the boundring box row indexed by the variable i
          # second input is a tensor of multiple rows of boundring boxes
          # the output is tensor containing IoUs of the boundring box
          # IOU(Box 1, Box 2)
...
# Takes a OpenCV images and coverts to the input of network
